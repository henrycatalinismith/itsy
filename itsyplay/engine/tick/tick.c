  ...
  while (SDL_PollEvent(&event)) {
    switch (event.type) {
      case SDL_FINGERDOWN:
        ...
        poke(TOUCH_0_B, true);
        poke(TOUCH_0_X, (int) floor(event.tfinger.x * 128));
        poke(TOUCH_0_Y, (int) floor(event.tfinger.y * 128));
        break;
      ...
      case SDL_FINGERMOTION:
        printf("SDL_FINGERMOTION %d, %d\n", (int) floor(event.tfinger.x * 128), (int) floor(event.tfinger.y * 128));
        poke(TOUCH_0_X, (int) floor(event.tfinger.x * 128));
        poke(TOUCH_0_Y, (int) floor(event.tfinger.y * 128));
        break;
